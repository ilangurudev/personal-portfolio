...
### 3.1 `albumTitleMap` Creation (4 locations) — **Done**

**Problem:** Each page recreates the album slug → title map, duplicating the `getCollection('albums')` lookup and `Map` creation.

**Files Affected:**
- `src/pages/photography/index.astro` (lightbox data)
- `src/pages/photography/photos.astro` (lightbox + React props)
- `src/pages/photography/tag/[tag].astro` (lightbox + React props)
- `src/pages/photography/album/[slug].astro` (lightbox + React props)

**Resolution:** Added `getAlbumTitleMap()` to `src/utils/photo-helpers.ts` (accepts optional preloaded albums to avoid duplicate queries) and consumed it in all four pages.
...
- `src/pages/photography/index.astro` → `const albumTitleMap = await getAlbumTitleMap();`
- `src/pages/photography/photos.astro` → `const albumTitleMap = await getAlbumTitleMap(albums);`
- `src/pages/photography/tag/[tag].astro` → `const albumTitleMap = await getAlbumTitleMap();`
- `src/pages/photography/album/[slug].astro` → `const albumTitleMap = await getAlbumTitleMap([album]);`

---

### 3.2 Filter Collapse/Expand Logic (3 locations) — **Done**

**Problem:** Each tag filter bar owned its own DOM toggles to expand/collapse the pill container.

**Resolution:** Centralized inside `src/components/photo/TagFilterBar.astro`; a single listener now controls the expand/collapse behavior that album + tag pages share.
...
**Files Covered:** `TagFilterBar.astro` (consumed by `album/[slug].astro` and `tag/[tag].astro`), removing per-page collapse scripts.

---

### 3.3 Clear Filters Visibility Logic (3 locations) — **Done**

**Problem:** Show/hide thresholds and reset handling for the “Clear filters” button were reimplemented per page.

**Resolution:** `TagFilterBar.astro` now owns the visibility + reset logic, including the “must keep one tag selected” rule on tag pages and the downstream event dispatch.
...
clearFiltersBtn?.addEventListener('click', (event) => {
  ...
  // Re-select initial tag on tag pages
  if (initialActiveTag) {
    const initialPill = tagPills.find(
      (pill) => normalize(pill.getAttribute('data-tag')) === normalize(initialActiveTag)
    );
    if (initialPill) initialPill.classList.add('active');
  }
...
**Files Covered:** Shared via `TagFilterBar.astro`; album + tag pages use the component without duplicating clear-button logic.
